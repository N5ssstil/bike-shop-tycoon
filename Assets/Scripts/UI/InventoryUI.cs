...
namespace BikeShopTycoon.UI
{
    ...
    public class InventoryUI : MonoBehaviour
    {
        ...
        // 内部库存列表（使用 Item 对象 + 数量/价格的元组）
        private List<SimpleStockEntry> currentInventory = new List<SimpleStockEntry>();
        private Dictionary<string, List<string>> brandItemsMap = new Dictionary<string, List<string>>();

        /// <summary>
        /// 简化的库存条目（仅用于 UI 显示）
        /// </summary>
        private class SimpleStockEntry
        {
            public string Name;
            public int Quantity;
            public int Price;
        ...
        private void AddToInventory(string itemName, int quantity, int price)
        {
            ...
            {
                currentInventory.Add(new SimpleStockEntry
                {
                    Name = itemName,
                    Quantity = quantity,
                    Price = price
        ...
        private void RefreshInventoryList()
        {
            ...
            foreach (var item in currentInventory)
            {
                if (inventoryItemPrefab != null)
                {
                    ...
                    if (tmp != null)
                    {
                        tmp.text = $"{item.Name} x{item.Quantity} - ¥{item.Price:N0}/件";
                    }
        ...
        private void LoadInventoryData()
        {
            ...
            currentInventory.Add(new SimpleStockEntry { Name = "入门铝合金公路车", Quantity = 2, Price = 2000 });
            currentInventory.Add(new SimpleStockEntry { Name = "通勤车", Quantity = 3, Price = 1500 });
            RefreshInventoryList();
        }
